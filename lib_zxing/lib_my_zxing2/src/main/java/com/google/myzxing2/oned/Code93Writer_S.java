 ...
 */
public class Code93Writer_S extends com.google.myzxing2.oned.OneDimensionalCodeWriter_S {
  @Override
  public BitMatrix_S encode(String contents,
    ...
    if (format != BarcodeFormat_S.CODE_93) {
  ...
  public boolean[] encode(String contents) {
    ...
    //start character (*)
    int pos = appendPattern(result, 0, Code93Reader_S.ASTERISK_ENCODING);

    for (int i = 0; i < length; i++) {
      int indexInString = Code93Reader_S.ALPHABET_STRING.indexOf(contents.charAt(i));
      pos += appendPattern(result, pos, Code93Reader_S.CHARACTER_ENCODINGS[indexInString]);
    }

    //add two checksums
    int check1 = computeChecksumIndex(contents, 20);
    pos += appendPattern(result, pos, Code93Reader_S.CHARACTER_ENCODINGS[check1]);

    //append the contents to reflect the first checksum added
    contents += Code93Reader_S.ALPHABET_STRING.charAt(check1);

    int check2 = computeChecksumIndex(contents, 15);
    pos += appendPattern(result, pos, Code93Reader_S.CHARACTER_ENCODINGS[check2]);

    //end character (*)
    pos += appendPattern(result, pos, Code93Reader_S.ASTERISK_ENCODING);
