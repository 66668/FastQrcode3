...
package com.google.myzxing2.pdf417.detector;
...
public final class Detector_S {
   ...
   * @return {@link PDF417DetectorResult_S} encapsulating results of detecting a PDF417 code
   * @throws NotFoundException_S if no PDF417 Code can be found
   */
  public static com.google.myzxing2.pdf417.detector.PDF417DetectorResult_S detect(BinaryBitmap_S image, Map<DecodeHintType_S,?> hints, boolean multiple)
      throws NotFoundException_S {
    // TODO detection improvement, tryHarder could try several different luminance thresholds/blackpoints or even 
    // different binarizers
    //boolean tryHarder = hints != null && hints.containsKey(DecodeHintType.TRY_HARDER);

    BitMatrix_S bitMatrix = image.getBlackMatrix();

    List<ResultPoint_S[]> barcodeCoordinates = detect(multiple, bitMatrix);
    if (barcodeCoordinates.isEmpty()) {
      bitMatrix = bitMatrix.clone();
      bitMatrix.rotate180();
      barcodeCoordinates = detect(multiple, bitMatrix);
    }
    return new com.google.myzxing2.pdf417.detector.PDF417DetectorResult_S(bitMatrix, barcodeCoordinates);
  }
   ...
   */
  private static List<ResultPoint_S[]> detect(boolean multiple, BitMatrix_S bitMatrix) {
    List<ResultPoint_S[]> barcodeCoordinates = new ArrayList<>();
    int row = 0;
    int column = 0;
    boolean foundBarcodeInRow = false;
    while (row < bitMatrix.getHeight()) {
      ResultPoint_S[] vertices = findVertices(bitMatrix, row, column);
      ...
      if (vertices[0] == null && vertices[3] == null) {
        ...
        column = 0;
        for (ResultPoint_S[] barcodeCoordinate : barcodeCoordinates) {
   ...
   */
  private static ResultPoint_S[] findVertices(BitMatrix_S matrix, int startRow, int startColumn) {
    int height = matrix.getHeight();
    int width = matrix.getWidth();

    ResultPoint_S[] result = new ResultPoint_S[8];
  ...
  private static void copyToResult(ResultPoint_S[] result, ResultPoint_S[] tmpResult, int[] destinationIndexes) {
    for (int i = 0; i < destinationIndexes.length; i++) {
      result[destinationIndexes[i]] = tmpResult[i];
    }
  }

  private static ResultPoint_S[] findRowsWithPattern(BitMatrix_S matrix,
                                                     int height,
                                                     int width,
                                                     int startRow,
                                                     int startColumn,
                                                     int[] pattern) {
    ResultPoint_S[] result = new ResultPoint_S[4];
    boolean found = false;
    ...
    for (; startRow < height; startRow += ROW_STEP) {
      ...
      if (loc != null) {
        ...
        result[0] = new ResultPoint_S(loc[0], startRow);
        result[1] = new ResultPoint_S(loc[1], startRow);
        found = true;
        break;
      }
    }
    ...
    if (found) {
      ...
      result[2] = new ResultPoint_S(previousRowLoc[0], stopRow);
      result[3] = new ResultPoint_S(previousRowLoc[1], stopRow);
    }
   ...
   */
  private static int[] findGuardPattern(BitMatrix_S matrix,
                                        int column,
                                        int row,
                                        int width,
                                        boolean whiteFirst,
