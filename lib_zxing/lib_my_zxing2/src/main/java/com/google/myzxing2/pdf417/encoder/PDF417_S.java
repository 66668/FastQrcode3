...
public final class PDF417_S {
  ...
  private BarcodeMatrix_S barcodeMatrix;
  private boolean compact;
  private Compaction_S compaction;
  ...
  public PDF417_S(boolean compact) {
    this.compact = compact;
    compaction = Compaction_S.AUTO;
  ...
  public BarcodeMatrix_S getBarcodeMatrix() {
  ...
  private static void encodeChar(int pattern, int len, BarcodeRow_S logic) {
    int map = 1 << len - 1;
   ...
   * @throws WriterException_S if the contents cannot be encoded in this format
   */
  public void generateBarcodeLogic(String msg, int errorCorrectionLevel) throws WriterException_S {

    //1. step: High-level encoding
    int errorCorrectionCodeWords = PDF417ErrorCorrection_S.getErrorCorrectionCodewordCount(errorCorrectionLevel);
    String highLevel = PDF417HighLevelEncoder_S.encodeHighLevel(msg, compaction, encoding);
    ...
    String ec = PDF417ErrorCorrection_S.generateErrorCorrection(dataCodewords, errorCorrectionLevel);

    //4. step: low-level encoding
    barcodeMatrix = new BarcodeMatrix_S(rows, cols);
   ...
   */
  private int[] determineDimensions(int sourceCodeWords, int errorCorrectionCodeWords) throws WriterException_S {
    float ratio = 0.0f;
    int[] dimension = null;
    ...
    if (dimension == null) {
      throw new WriterException_S("Unable to fit message in columns");
    }
   ...
   */
  public void setCompaction(Compaction_S compaction) {
