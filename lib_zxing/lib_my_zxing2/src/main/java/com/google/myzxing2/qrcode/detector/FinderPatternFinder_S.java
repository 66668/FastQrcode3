...
package com.google.myzxing2.qrcode.detector;
 ...
 * @author Sean Owen
 */
public class FinderPatternFinder_S {
  ...
  private final List<FinderPattern_S> possibleCenters;
  private boolean hasSkipped;
  private final int[] crossCheckStateCount;
  private final ResultPointCallback_S resultPointCallback;
   ...
   */
  public FinderPatternFinder_S(BitMatrix_S image) {
    this(image, null);
  }

  public FinderPatternFinder_S(BitMatrix_S image, ResultPointCallback_S resultPointCallback) {
    this.image = image;
  ...
  protected final List<FinderPattern_S> getPossibleCenters() {
    return possibleCenters;
  }

  final com.google.myzxing2.qrcode.detector.FinderPatternInfo_S find(Map<DecodeHintType_S,?> hints) throws NotFoundException_S {
    ...
    FinderPattern_S[] patternInfo = selectBestPatterns();
    ResultPoint_S.orderBestPatterns(patternInfo);

    return new com.google.myzxing2.qrcode.detector.FinderPatternInfo_S(patternInfo);
  }
    ...
    if (!Float.isNaN(centerI)) {
      ...
      if (!Float.isNaN(centerJ) && crossCheckDiagonal((int) centerI, (int) centerJ)) {
        ...
        for (int index = 0; index < possibleCenters.size(); index++) {
          FinderPattern_S center = possibleCenters.get(index);
        ...
        if (!found) {
          FinderPattern_S point = new FinderPattern_S(centerJ, centerI, estimatedModuleSize);
  ...
  private int findRowSkip() {
    ...
    ResultPoint_S firstConfirmedCenter = null;
    for (FinderPattern_S center : possibleCenters) {
  ...
  private boolean haveMultiplyConfirmedCenters() {
    ...
    int max = possibleCenters.size();
    for (FinderPattern_S pattern : possibleCenters) {
   ...
   */
  private FinderPattern_S[] selectBestPatterns() throws NotFoundException_S {
    ...
    if (startSize > 3) {
      ...
      for (int i = 0; i < possibleCenters.size() && possibleCenters.size() > 3; i++) {
        FinderPattern_S pattern = possibleCenters.get(i);
    ...
    if (possibleCenters.size() > 3) {
      ...
      float totalModuleSize = 0.0f;
      for (FinderPattern_S possibleCenter : possibleCenters) {
   ...
   */
  private static final class FurthestFromAverageComparator implements Comparator<FinderPattern_S>, Serializable {
  ...
  /**
   * <p>Orders by {@link FinderPattern_S#getCount()}, descending.</p>
   */
  private static final class CenterComparator implements Comparator<FinderPattern_S>, Serializable {
    private final float average;
