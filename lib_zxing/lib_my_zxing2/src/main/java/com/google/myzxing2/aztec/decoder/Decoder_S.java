...
package com.google.myzxing2.aztec.decoder;
...
public final class Decoder_S {
  ...
  private AztecDetectorResult_S ddata;

  public DecoderResult_S decode(AztecDetectorResult_S detectorResult) throws FormatException_S {
    ddata = detectorResult;
    BitMatrix_S matrix = detectorResult.getBits();
    boolean[] rawbits = extractBits(matrix);
    boolean[] correctedBits = correctBits(rawbits);
    byte[] rawBytes = convertBoolArrayToByteArray(correctedBits);
    String result = getEncodedData(correctedBits);
    DecoderResult_S decoderResult = new DecoderResult_S(rawBytes, result, null, null);
  ...
  /**
   * Gets the string_a encoded in the aztec code bits
   *
   * @return the decoded string_a
   */
  ...
  /**
   * Gets the character (or string_a) corresponding to the passed code in the given table
   *
   ...
   * @throws FormatException_S if the input contains too many errors
   */
  private boolean[] correctBits(boolean[] rawbits) throws FormatException_S {
    GenericGF_S gf;
    int codewordSize;

    if (ddata.getNbLayers() <= 2) {
      codewordSize = 6;
      gf = GenericGF_S.AZTEC_DATA_6;
    } else if (ddata.getNbLayers() <= 8) {
      codewordSize = 8;
      gf = GenericGF_S.AZTEC_DATA_8;
    } else if (ddata.getNbLayers() <= 22) {
      codewordSize = 10;
      gf = GenericGF_S.AZTEC_DATA_10;
    } else {
      codewordSize = 12;
      gf = GenericGF_S.AZTEC_DATA_12;
    }
    ...
    try {
      ReedSolomonDecoder_S rsDecoder = new ReedSolomonDecoder_S(gf);
      rsDecoder.decode(dataWords, numCodewords - numDataCodewords);
    } catch (ReedSolomonException_S ex) {
      throw FormatException_S.getFormatInstance(ex);
    }
   ...
   */
  private boolean[] extractBits(BitMatrix_S matrix) {
