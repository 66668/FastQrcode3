 ...
 * @author Sean Owen
 */
public final class URIResultParser_S extends ResultParser_S {
  ...
  public URIParsedResult_S parse(Result_S result) {
    ...
    if (rawText.startsWith("URL:") || rawText.startsWith("URI:")) {
      return new URIParsedResult_S(rawText.substring(4).trim(), null);
    }
    rawText = rawText.trim();
    if (!isBasicallyValidURI(rawText) || isPossiblyMaliciousURI(rawText)) {
      return null;
    }
    return new URIParsedResult_S(rawText, null);
  }
