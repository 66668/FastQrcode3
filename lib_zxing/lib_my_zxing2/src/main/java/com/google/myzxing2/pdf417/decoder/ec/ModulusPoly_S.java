...
/**
 * @author Sean Owen
 * @see com.google.myzxing2.common.reedsolomon.GenericGFPoly
 */
final class ModulusPoly_S {

  private final ModulusGF_S field;
  private final int[] coefficients;

  ModulusPoly_S(ModulusGF_S field, int[] coefficients) {
  ...
  ModulusPoly_S add(ModulusPoly_S other) {
    ...
    return new ModulusPoly_S(field, sumDiff);
  }

  ModulusPoly_S subtract(ModulusPoly_S other) {
    if (!field.equals(other.field)) {
      throw new IllegalArgumentException("ModulusPolys do not have same ModulusGF field");
    }
    if (other.isZero()) {
  ...
  ModulusPoly_S multiply(ModulusPoly_S other) {
    if (!field.equals(other.field)) {
      throw new IllegalArgumentException("ModulusPolys do not have same ModulusGF field");
    }
    ...
    return new ModulusPoly_S(field, product);
  }

  ModulusPoly_S negative() {
    int size = coefficients.length;
    int[] negativeCoefficients = new int[size];
    for (int i = 0; i < size; i++) {
      negativeCoefficients[i] = field.subtract(0, coefficients[i]);
    }
    return new ModulusPoly_S(field, negativeCoefficients);
  }

  ModulusPoly_S multiply(int scalar) {
    if (scalar == 0) {
      return field.getZero();
    }
    if (scalar == 1) {
  ...
  ModulusPoly_S multiplyByMonomial(int degree, int coefficient) {
    if (degree < 0) {
      throw new IllegalArgumentException();
    }
    if (coefficient == 0) {
