 ...
 */
public final class EAN13Writer_S extends com.google.myzxing2.oned.UPCEANWriter_S {
  ...
  public boolean[] encode(String contents) {
    ...
    switch (length) {
      case 12:
        ...
        int check;
        try {
          check = com.google.myzxing2.oned.UPCEANReader_S.getStandardUPCEANChecksum(contents);
      ...
      case 13:
        try {
          if (!com.google.myzxing2.oned.UPCEANReader_S.checkStandardUPCEANChecksum(contents)) {
    ...
    pos += appendPattern(result, pos, com.google.myzxing2.oned.UPCEANReader_S.START_END_PATTERN, true);
    ...
    for (int i = 1; i <= 6; i++) {
      ...
      pos += appendPattern(result, pos, com.google.myzxing2.oned.UPCEANReader_S.L_AND_G_PATTERNS[digit], false);
    }

    pos += appendPattern(result, pos, com.google.myzxing2.oned.UPCEANReader_S.MIDDLE_PATTERN, false);

    for (int i = 7; i <= 12; i++) {
      int digit = Character.digit(contents.charAt(i), 10);
      pos += appendPattern(result, pos, com.google.myzxing2.oned.UPCEANReader_S.L_PATTERNS[digit], true);
    }
    appendPattern(result, pos, com.google.myzxing2.oned.UPCEANReader_S.START_END_PATTERN, true);
