...
/**
 * This object renders an UPC-E code as a {@link BitMatrix_S}.
 *
 * @author 0979097955s@gmail.com (RX)
 */
public final class UPCEWriter_S extends UPCEANWriter_S {
  ...
  public BitMatrix_S encode(String contents,
    ...
    if (format != BarcodeFormat_S.UPC_E) {
  ...
  public boolean[] encode(String contents) {
    ...
    switch (length) {
      case 7:
        ...
        int check;
        try {
          check = UPCEANReader_S.getStandardUPCEANChecksum(UPCEReader_S.convertUPCEtoUPCA(contents));
      ...
      case 8:
        try {
          if (!UPCEANReader_S.checkStandardUPCEANChecksum(contents)) {
    ...
    int parities = UPCEReader_S.NUMSYS_AND_CHECK_DIGIT_PATTERNS[firstDigit][checkDigit];
    boolean[] result = new boolean[CODE_WIDTH];
    int pos = 0;

    pos += appendPattern(result, pos, UPCEANReader_S.START_END_PATTERN, true);
    ...
    for (int i = 1; i <= 6; i++) {
      ...
      pos += appendPattern(result, pos, UPCEANReader_S.L_AND_G_PATTERNS[digit], false);
    }

    appendPattern(result, pos, UPCEANReader_S.END_PATTERN, false);
