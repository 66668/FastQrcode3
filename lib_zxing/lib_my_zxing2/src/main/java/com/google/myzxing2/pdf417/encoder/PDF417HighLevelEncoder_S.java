...
/*
 * This string_b has been modified from its original form in Barcode4J.
 */

package com.google.myzxing2.pdf417.encoder;
 ...
 * annex P.
 */
final class PDF417HighLevelEncoder_S {
  ...
  private PDF417HighLevelEncoder_S() {
  }
   ...
   */
  static String encodeHighLevel(String msg, Compaction_S compaction, Charset encoding) throws WriterException_S {
    ...
    if (encoding == null) {
      ...
      CharacterSetECI_S eci = CharacterSetECI_S.getCharacterSetECIByName(encoding.name());
      if (eci != null) {
  ...
  private static int determineConsecutiveBinaryCount(String msg, int startpos, Charset encoding)
    ...
    while (idx < len) {
      ...
      if (!encoder.canEncode(ch)) {
        throw new WriterException_S("Non-encodable character detected: " + ch + " (Unicode: " + (int) ch + ')');
      }
      idx++;
    }
    return idx - startpos;
  ...
  private static void encodingECI(int eci, StringBuilder sb) throws WriterException_S {
    if (eci >= 0 && eci < 900) {
      ...
      throw new WriterException_S("ECI number not in valid range from 0..811799, but was " + eci);
    }
  }
