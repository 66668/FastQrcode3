...
final class EdifactEncoder_S implements Encoder_S {
  ...
  public int getEncodingMode() {
    return com.google.myzxing3.datamatrix.encoder.HighLevelEncoder_S.EDIFACT_ENCODATION;
  }
   ...
   */
  private static void handleEOD(com.google.myzxing3.datamatrix.encoder.EncoderContext_S context, CharSequence buffer) {
    try {
      int count = buffer.length();
      if (count == 0) {
  ...
  private static void encodeChar(char c, StringBuilder sb) {
    if (c >= ' ' && c <= '?') {
      ...
      com.google.myzxing3.datamatrix.encoder.HighLevelEncoder_S.illegalCharacter(c);
    }
  }
