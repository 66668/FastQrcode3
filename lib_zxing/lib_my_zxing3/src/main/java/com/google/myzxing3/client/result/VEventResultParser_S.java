 ...
 * @author Sean Owen
 */
public final class VEventResultParser_S extends ResultParser_S {

  @Override
  public CalendarParsedResult_S parse(Result_S result) {
    ...
    try {
      return new CalendarParsedResult_S(summary,
                                      start,
                                      end,
                                      duration,
                                      location,
  ...
  private static String matchSingleVCardPrefixedField(CharSequence prefix,
                                                      String rawText,
                                                      boolean trim) {
    List<String> values = VCardResultParser_S.matchSingleVCardPrefixedField(prefix, rawText, trim, false);
    return values == null || values.isEmpty() ? null : values.get(0);
  }

  private static String[] matchVCardPrefixedField(CharSequence prefix, String rawText, boolean trim) {
    List<List<String>> values = VCardResultParser_S.matchVCardPrefixedField(prefix, rawText, trim, false);
