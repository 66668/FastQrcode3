...
package com.google.myzxing3.datamatrix.decoder;
...
final class BitMatrixParser_S {
  ...
  private final BitMatrix_S mappingBitMatrix;
  private final BitMatrix_S readMappingMatrix;
  private final Version_S version;

  /**
   * @param bitMatrix {@link BitMatrix_S} to parse
   * @throws FormatException_S if dimension is < 8 or > 144 or not 0 mod 2
   */
  BitMatrixParser_S(BitMatrix_S bitMatrix) throws FormatException_S {
    ...
    this.readMappingMatrix = new BitMatrix_S(this.mappingBitMatrix.getWidth(), this.mappingBitMatrix.getHeight());
  }

  Version_S getVersion() {
    return version;
  }
   ...
   *
   * @param bitMatrix Original {@link BitMatrix_S} including alignment patterns
   * @return {@link Version_S} encapsulating the Data Matrix Code's "version"
   * @throws FormatException_S if the dimensions of the mapping matrix are not valid
   * Data Matrix dimensions.
   */
  private static Version_S readVersion(BitMatrix_S bitMatrix) throws FormatException_S {
    int numRows = bitMatrix.getHeight();
    int numColumns = bitMatrix.getWidth();
    return com.google.myzxing3.datamatrix.decoder.Version_S.getVersionForDimensions(numRows, numColumns);
  }

  /**
   * <p>Reads the bits in the {@link BitMatrix_S} representing the mapping matrix (No alignment patterns)
  ...
  /**
   * <p>Extracts the data region from a {@link BitMatrix_S} that contains
   * alignment patterns.</p>
   *
   * @param bitMatrix Original {@link BitMatrix_S} with alignment patterns
   * @return BitMatrix that has the alignment patterns removed
   */
  private BitMatrix_S extractDataRegion(BitMatrix_S bitMatrix) {
    ...
    BitMatrix_S bitMatrixWithoutAlignment = new BitMatrix_S(sizeDataRegionColumn, sizeDataRegionRow);
