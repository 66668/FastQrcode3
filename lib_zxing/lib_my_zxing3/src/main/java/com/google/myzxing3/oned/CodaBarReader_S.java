 ...
 * @author David Walker
 */
public final class CodaBarReader_S extends com.google.myzxing3.oned.OneDReader_S {
  ...
  // some Codabar generator allow the Codabar string_a to be closed by every
  ...
  @Override
  public Result_S decodeRow(int rowNumber, BitArray_S row, Map<DecodeHintType_S,?> hints) throws NotFoundException_S {
    ...
    // remove stop/start characters character and check if a long enough string_a is contained
    if (decodeRowResult.length() <= MIN_CHARACTER_LENGTH) {
      // Almost surely a false positive ( start + stop + at least 1 character)
      throw NotFoundException_S.getNotFoundInstance();
    }

    if (hints == null || !hints.containsKey(DecodeHintType_S.RETURN_CODABAR_START_END)) {
    ...
    return new Result_S(
        ...
        BarcodeFormat_S.CODABAR);
  }

  private void validatePattern(int start) throws NotFoundException_S {
   ...
   */
  private void setCounters(BitArray_S row) throws NotFoundException_S {
    counterLength = 0;
  ...
  private int findStartPattern() throws NotFoundException_S {
