 ...
 */
public final class ITFReader_S extends com.google.myzxing3.oned.OneDReader_S {
  ...
  public Result_S decodeRow(int rowNumber, BitArray_S row, Map<DecodeHintType_S,?> hints)
      throws FormatException_S, NotFoundException_S {
    ...
    if (hints != null) {
      allowedLengths = (int[]) hints.get(DecodeHintType_S.ALLOWED_LENGTHS);
    ...
    // an assumption that the decoded string_a must be a 'standard' length if it's short
    ...
    return new Result_S(
        ...
        new ResultPoint_S[] {new ResultPoint_S(startRange[1], rowNumber),
                           new ResultPoint_S(endRange[0], rowNumber)},
        BarcodeFormat_S.ITF);
  }
   ...
   */
  private static void decodeMiddle(BitArray_S row,
                                   int payloadStart,
                                   int payloadEnd,
   ...
   *         'start block'
   */
  private int[] decodeStart(BitArray_S row) throws NotFoundException_S {
   ...
   * @throws NotFoundException_S if the quiet zone cannot be found
   */
  private void validateQuietZone(BitArray_S row, int startPattern) throws NotFoundException_S {
   ...
   * @throws NotFoundException_S Throws exception if no black lines are found in the row
   */
  private static int skipWhiteSpace(BitArray_S row) throws NotFoundException_S {
   ...
   *         block'
   */
  private int[] decodeEnd(BitArray_S row) throws NotFoundException_S {
   ...
   */
  private static int decodeDigit(int[] counters) throws NotFoundException_S {
