 ...
 */
public final class EAN13Writer_S extends UPCEANWriter_S {
  ...
  public boolean[] encode(String contents) {
    ...
    switch (length) {
      case 12:
        ...
        int check;
        try {
          check = UPCEANReader_S.getStandardUPCEANChecksum(contents);
    ...
    pos += appendPattern(result, pos, UPCEANReader_S.MIDDLE_PATTERN, false);

    for (int i = 7; i <= 12; i++) {
      int digit = Character.digit(contents.charAt(i), 10);
      pos += appendPattern(result, pos, UPCEANReader_S.L_PATTERNS[digit], true);
    }
    appendPattern(result, pos, UPCEANReader_S.START_END_PATTERN, true);
