...
import com.google.myzxing.common.PerspectiveTransform_S;
...
public class Detector_S {
  ...
  public final DetectorResult_S detect(Map<DecodeHintType_S,?> hints) throws NotFoundException_S, FormatException_S {
    ...
    FinderPatternFinder_S finder = new FinderPatternFinder_S(image, resultPointCallback);
    FinderPatternInfo_S info = finder.find(hints);

    return processFinderPatternInfo(info);
  }

  protected final DetectorResult_S processFinderPatternInfo(FinderPatternInfo_S info)
      throws NotFoundException_S, FormatException_S {

    FinderPattern_S topLeft = info.getTopLeft();
    FinderPattern_S topRight = info.getTopRight();
    FinderPattern_S bottomLeft = info.getBottomLeft();
