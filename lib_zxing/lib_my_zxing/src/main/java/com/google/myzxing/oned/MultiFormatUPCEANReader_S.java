 ...
 * @author Sean Owen
 */
public final class MultiFormatUPCEANReader_S extends OneDReader_S {

  private static final UPCEANReader_S[] EMPTY_READER_ARRAY = new UPCEANReader_S[0];

  private final UPCEANReader_S[] readers;
  ...
  public MultiFormatUPCEANReader_S(Map<DecodeHintType_S,?> hints) {
    ...
    Collection<UPCEANReader_S> readers = new ArrayList<>();
  ...
  public Result_S decodeRow(int rowNumber,
    ...
    int[] startGuardPattern = UPCEANReader_S.findStartGuardPattern(row);
    for (UPCEANReader_S reader : readers) {
      try {
