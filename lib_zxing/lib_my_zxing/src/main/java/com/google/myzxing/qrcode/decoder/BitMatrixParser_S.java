...
final class BitMatrixParser_S {
  ...
  private FormatInformation_S parsedFormatInfo;
   ...
   */
  FormatInformation_S readFormatInformation() throws FormatException_S {
    ...
    parsedFormatInfo = FormatInformation_S.decodeFormatInformation(formatInfoBits1, formatInfoBits2);
    if (parsedFormatInfo != null) {
      return parsedFormatInfo;
    }
    throw FormatException_S.getFormatInstance();
  ...
  Version_S readVersion() throws FormatException_S {
    ...
    if (provisionalVersion <= 6) {
      return Version_S.getVersionForNumber(provisionalVersion);
    }
    ...
    Version_S theParsedVersion = Version_S.decodeVersionInformation(versionBits);
    ...
    theParsedVersion = Version_S.decodeVersionInformation(versionBits);
    if (theParsedVersion != null && theParsedVersion.getDimensionForVersion() == dimension) {
      parsedVersion = theParsedVersion;
      return theParsedVersion;
    }
  ...
  byte[] readCodewords() throws FormatException_S {
    ...
    FormatInformation_S formatInfo = readFormatInformation();
    Version_S version = readVersion();

    // Get the data mask for the format used in this QR Code. This will exclude
    // some bits from reading as we wind through the bit matrix.
    DataMask_S dataMask = DataMask_S.values()[formatInfo.getDataMask()];
    int dimension = bitMatrix.getHeight();
  ...
  void remask() {
    ...
    DataMask_S dataMask = DataMask_S.values()[parsedFormatInfo.getDataMask()];
    int dimension = bitMatrix.getHeight();
    dataMask.unmaskBitMatrix(bitMatrix, dimension);
  }
